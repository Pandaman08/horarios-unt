...
router.get('/', async (req, res) => {
  try {
    const id_periodo_str = req.query.id_periodo as string;
    const id_periodo = id_periodo_str ? parseInt(id_periodo_str, 10) : NaN;
    ...
    gruposEnPeriodo.forEach((g: any) => {
      g.curso.docente_cursos.forEach((dc: any) => {
    ...
    res.json({
      ...
      kpis: {
        totalDocentes: totalDocentesEnPeriodo,
        docentesAtendidos,
