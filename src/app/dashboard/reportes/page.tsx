  ...
  return (
    <div className="p-4 md:p-6 max-w-[1800px] mx-auto space-y-6 pb-8 animate-in fade-in duration-500">
      ...
      <div className="bg-card p-5 rounded-2xl border border-border shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-4 bg-muted/50 p-3 rounded-2xl border border-border w-full md:w-auto">
          <div className="flex flex-col">
            <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Periodo Académico</span>
            <div className="relative min-w-[200px] mt-1">
              <select 
                value={selectedPeriodo}
                onChange={(e) => setSelectedPeriodo(e.target.value)}
                className="w-full bg-card border border-border rounded-xl px-4 py-2 text-xs text-foreground font-bold outline-none focus:ring-2 focus:ring-primary transition-all appearance-none cursor-pointer"
              >
                {periodos.map(p => (
