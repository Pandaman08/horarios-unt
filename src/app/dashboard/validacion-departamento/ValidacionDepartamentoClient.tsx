...
const getEstadoBadge = (estado: string) => {
  switch (estado) {
    ...
    case 'VALIDADO_DEPARTAMENTO':
      return <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800 px-3 py-1 font-semibold uppercase text-[10px] tracking-wider">Validado</Badge>;
    case 'APROBADO':
...
export default function ValidacionDepartamentoClient({ periodos }: { periodos: any[] }) {
  ...
  const handleValidar = async (declaracionId: number) => {
    try {
      const res = await fetch(`/api/validacion-departamento/${declaracionId}`, {
        method: 'PUT',
      ...
      if (!res.ok) {
        ...
        toast.error(error.error || 'Error al validar la declaración');
        return;
      }
      toast.success('Declaración validada correctamente');
      ...
      console.error(err);
      toast.error('Error al validar la declaración');
    }
  };
        ...
        <CardContent className="p-0">
          <div className="p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-border bg-card">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-lg font-bold text-foreground leading-none">Validación de Departamento</h1>
                <p className="text-muted-foreground text-xs mt-1">Validación inicial de declaraciones de carga horaria</p>
              </div>
            </div>
            <div className="flex items-center gap-2 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 px-3 py-1.5 rounded-md text-xs font-semibold border border-blue-100 dark:border-blue-800">
          ...
          <div className="p-3 bg-muted/30">
            <div className="relative max-w-md">
              ...
              <Input
                ...
                className="pl-9 h-10 bg-background border-border focus:ring-1 focus:ring-blue-500 dark:focus:ring-blue-400 rounded-md text-sm shadow-sm"
              />
            </div>
          </div>
        </CardContent>
                ...
                currentDeclaraciones.map((declaracion) => {
                  ...
                  return (
                    <TableRow key={declaracion.id_declaracion} className="hover:bg-muted/50 transition-colors border-b border-border last:border-0">
                      ...
                      <TableCell className="px-6 py-4">
                        <Badge variant="outline" className="bg-muted text-muted-foreground border-border text-[9px] font-bold uppercase px-2 py-0.5">
                              ...
                              <div className="pt-6 border-t border-border space-y-4">
                                <div className="space-y-2">
                                  <Label className="text-xs font-bold text-foreground uppercase tracking-wider flex items-center gap-2">
                                    ...
                                    Observaciones (Opcional si valida, obligatorio si rechaza)
                                  </Label>
                                  <Textarea
                                ...
                                <div className="flex justify-end gap-3">
                                  ...
                                  <Button
                                    onClick={() => handleValidar(declaracion.id_declaracion)}
