...
// Tipo inferido para DocenteCurso con curso y sus grupos
type DocenteCursoWithGrupos = Prisma.DocenteCursoGetPayload<{
  include: { curso: { include: { grupos: true } } }
}>;
...
export async function GET(request: Request) {
  try {
    ...
    // Si es admin/operador y proporciona un id_docente_manual, usar ese
    if (
      id_docente_manual &&
      (session.user.rol === 'administrador_sistema' || session.user.rol === 'operador_horarios')
    ) {
      const parsedDocenteId = parseInt(id_docente_manual);
      if (isNaN(parsedDocenteId)) {
        return NextResponse.json({ error: 'id_docente_manual debe ser un número válido' }, { status: 400 });
      }
      docenteId = parsedDocenteId;
    } else {
      // El docente autenticado busca su propio registro
      const docente = await prisma.docente.findFirst({
        where: { id_usuario: String(session.user.id_usuario) } // Aseguramos string
      });
      if (!docente) {
    ...
    // Obtener cursos asignados al docente activos, incluyendo grupos del período
    ...
    // Filtrar solo aquellos que tengan al menos un grupo en el período
    const cursosConGrupos = cursosAsignados.filter(
      (dc: DocenteCursoWithGrupos) => dc.curso.grupos && dc.curso.grupos.length > 0
    );

    // Eliminar duplicados lógicos (mismo curso y tipo de clase)
    const uniqueCursos = Array.from(
      new Set(cursosConGrupos.map((c) => `${c.id_curso}-${c.tipo_clase.toLowerCase()}`))
    )
      .map((key) => {
        const [id, tipo] = key.split('-');
        return cursosConGrupos.find(
          (c) => c.id_curso === parseInt(id) && c.tipo_clase.toLowerCase() === tipo
        );
      })
      .filter((c): c is DocenteCursoWithGrupos => c !== undefined);

    // Calcular progreso para cada curso único
    const progreso = await Promise.all(
      uniqueCursos.map(async (dc: DocenteCursoWithGrupos) => {
        // Asignaciones confirmadas (horarios definitivos)
        ...
        // Selecciones temporales vigentes (no expiradas)
        ...
        const calcularMinutos = (hInicio: string | null, hFin: string | null): number => {
        ...
        if (tipo.includes('teoria')) horasRequeridas = dc.curso.horas_teoria ?? 0;
        else if (tipo.includes('laboratorio')) horasRequeridas = dc.curso.horas_laboratorio ?? 0;
