import { Docente, CondicionDocente, RegimenDedicacion } from "@prisma/client";
...
export function calcularDeclaracionJurada(docente: Docente): string {
  const { condicion, regimenDedicacion, tipoContrato, esInvestigadorAcreditado } = docente;
  ...
  if (condicion === CondicionDocente.EXTRAORDINARIO) {
    if (docente.tipoExtraordinario === null) {
