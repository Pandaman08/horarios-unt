...
export default function SeleccionHorariosPage() {
  ...
  const checkAccessOnce = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/api/horarios/check-interval`);
  ...
  const verificarHorariosGenerados = async () => {
    ...
    try {
      ...
      const resConfirmados = await fetch(`${apiUrl}/api/docentes/horarios?periodoId=${idPeriodo}`);
      ...
      const resTemporales = await fetch(`${apiUrl}/api/horarios/disponibilidad-matriz?id_periodo=${idPeriodo}&id_docente=${session.user.id_docente}`);
  ...
  const fetchDocenteCursos = async () => {
    ...
    try {
      ...
      const url = session?.user?.rol === 'docente' 
        ? `${apiUrl}/api/docentes/mis-cursos?id_periodo=${idPeriodo}`
        : `${apiUrl}/api/docentes/mis-cursos?id_periodo=${idPeriodo}&id_docente=${session?.user?.id_docente}`;
  ...
  const fetchGrupos = async () => {
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/cursos/${cursoSeleccionado.id}/grupos?periodoId=${idPeriodo}`);
      if (res.ok) {
        const data = await res.json();
        setGrupos(data);
  ...
  const confirmarTodo = async () => {
    ...
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
    const resCheck = await fetch(`${apiUrl}/api/horarios/disponibilidad-matriz?id_periodo=${idPeriodo}&id_docente=${session.user.id_docente}`);
