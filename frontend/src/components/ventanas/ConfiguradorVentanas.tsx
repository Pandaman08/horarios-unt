...
export function ConfiguradorVentanas() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const statsRes = await fetch(`${apiUrl}/api/ventanas?stats=true`);
  ...
  const fetchDisponibilidadDocente = async (id: number) => {
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/docentes/${id}/disponibilidad?id_periodo=${periodoSeleccionado.id_periodo}`);
  ...
  const fetchVentanas = async () => {
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/ventanas?id_periodo=${periodoSeleccionado.id_periodo}`);
      const data = await res.json();
  ...
  const handleAutoSchedule = async (e: React.FormEvent) => {
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/ventanas`, {
        method: "POST",
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/ventanas/${id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  const handleGenerarHorarios = async () => {
    ...
    try {
      ...
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/horarios/asignacion-automatica`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
