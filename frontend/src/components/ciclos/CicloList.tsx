...
export function CicloList() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editingCiclo 
      ? `${apiUrl}/api/ciclos/${editingCiclo.id_ciclo}` 
      : `${apiUrl}/api/ciclos`;
  ...
  const handleDelete = async (id: number) => {
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/ciclos/${id}`, { method: "DELETE" });
      const data = await res.json();
  ...
  const handleGenerateConsolidatedReport = async () => {
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const url = `${apiUrl}/api/reportes?tipo=ciclos_todos&id_periodo=${periodoSeleccionado.id_periodo}`;
