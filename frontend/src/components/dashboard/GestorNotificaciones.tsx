...
export function GestorNotificaciones() {
  ...
  const fetchData = async () => {
    try {
      ...
      const url = filtroVentana
        ? `${apiUrl}/api/notificaciones/admin?type=stats&id_ventana=${filtroVentana}`
        : `${apiUrl}/api/notificaciones/admin?type=stats`;
  ...
  const procesarColaManual = async () => {
    try {
      setProcessing(true);
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/notificaciones/admin`, {
        method: "POST",
  ...
  const handleRetry = async (id: number) => {
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/notificaciones/retry`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id }),
      });
      if (res.ok) { toast.success("Reintento enviado"); fetchData(); }
    } catch {
