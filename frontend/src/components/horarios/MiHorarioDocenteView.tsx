...
export function MiHorarioDocenteView() {
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      const url = `${apiUrl}/api/reportes/pdf?tipo=docente_propio&id_periodo=${selectedPeriodo}`;
  ...
  const fetchHorarios = async () => {
    try {
      setLoading(true);
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/docentes/horarios?periodoId=${selectedPeriodo}`);
