...
export function DocenteList() {
  ...
  const handleGenerateInstitutionalReport = async () => {
    ...
    try {
      ...
      const url = `${apiUrl}/api/reportes/pdf?tipo=reporte_docentes_lista&id_periodo=${periodoSeleccionado.id_periodo}`;
  ...
  const fetchCiclos = async () => {
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/ciclos`);
  ...
  const fetchDocentes = async () => {
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/docentes`);
      const contentType = res.headers.get("content-type");
  ...
  const handleGenerateReport = async (docente: Docente) => {
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const url = `${apiUrl}/api/reportes/pdf?tipo=docente&id_periodo=${periodoSeleccionado.id_periodo}&id=${docente.id_docente}`;
      const response = await fetch(url);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/docentes/${id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  const handleDocenteSubmit = async (e: React.FormEvent) => {
    ...
    const url = editingDocente 
      ? `${apiUrl}/api/docentes/${editingDocente.id_docente}` 
      : `${apiUrl}/api/docentes`;
