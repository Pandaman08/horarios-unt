...
export function PeriodoList() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editingPeriodo 
      ? `${apiUrl}/api/periodos/${editingPeriodo.id_periodo}` 
      : `${apiUrl}/api/periodos`;
  ...
  const handleDelete = async (id: number) => {
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/periodos/${id}`, { method: "DELETE" });
      const data = await res.json();
  ...
  const handleGenerateSelectedPeriodReport = async () => {
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const url = `${apiUrl}/api/reportes?tipo=reporte_periodos&id_periodo=${periodoSeleccionado.id_periodo}`;
