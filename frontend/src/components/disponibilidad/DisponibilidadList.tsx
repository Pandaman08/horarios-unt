...
export function DisponibilidadList() {
  ...
  const fetchDocentes = async () => {
    ...
    try {
      ...
      const url = `${apiUrl}/api/docentes/disponibilidad/listar?periodoId=${selectedPeriodo}&search=${searchTerm}&categoria=${categoria}&modalidad=${modalidad}&orden=${orden}`;
  ...
  const handleEditDisponibilidad = async (docente: DocenteDisp) => {
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/docentes/disponibilidad/${docente.id_docente}?periodoId=${selectedPeriodo}`);
      const data = await res.json();
  ...
  const handleSaveDisponibilidad = async (data: any[]) => {
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/docentes/disponibilidad`, {
        method: "POST",
