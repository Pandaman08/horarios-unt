...
export function VisorReportes() {
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/reportes/excel?id_periodo=${id_periodo}`);
  ...
  const fetchData = async () => {
    try {
      ...
      const [dRes, aRes, cRes] = await Promise.all([
        fetch(`${apiUrl}/api/docentes`),
        fetch(`${apiUrl}/api/ambientes`),
        fetch(`${apiUrl}/api/ciclos`)
      ]);
  ...
  const handleDownload = async (tipo: string, id?: string) => {
    ...
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
    let url = `${apiUrl}/api/reportes/pdf?tipo=${tipo}&id_periodo=${id_periodo}`;
    if (id) url += `&id=${id}`;
