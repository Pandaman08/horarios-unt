...
export function AmbienteList() {
  ...
  const handleDelete = async (id: number) => {
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const res = await fetch(`${apiUrl}/api/ambientes/${id}`, { method: "DELETE" });
      const data = await res.json();
  ...
  const handleGenerateConsolidatedReport = async () => {
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
      const url = `${apiUrl}/api/reportes?tipo=reporte_ambientes&id_periodo=${periodoSeleccionado.id_periodo}`;
